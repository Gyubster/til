...
이전에 설계한 클라이밍 앱의 AI 코칭을 고민하다가, 한 가지 더 떠오른 아이디어가 있다. 클라이밍 기록 앱에서 가장 귀찮은 부분은 **수동 입력**이다. 클라이밍이 끝나고 "빨강 3개, 파랑 5개, 초록 2개"를 일일이 입력하는 건 번거롭다. 대부분의 사용자는 3일이면 입력을 포기할 것이다.

그런데 클라이머들은 클라이밍 벽 사진을 거의 항상 찍는다. Vision API로 **사진에서 홀드 색상과 루트를 자동으로 인식**할 수 있다면 어떨까? 가능성을 검토해봤다.
