---
title: "클라이밍 사진에서 루트 자동 인식 - Vision API 적용기"
date: 2026-05-04
...
클라이밍 기록 앱에서 가장 귀찮은 부분은 **수동 입력**이다. 클라이밍이 끝나고 "빨강 3개, 파랑 5개, 초록 2개"를 일일이 입력하는 건 번거롭다. 대부분의 사용자는 3일이면 입력을 포기한다.

그런데 클라이머들은 클라이밍 벽 사진을 거의 항상 찍는다. **사진에서 홀드 색상과 루트를 자동으로 인식**할 수 있다면, 입력 부담이 크게 줄어든다.
...
### Vision API는 "완벽한 자동화"가 아니라 "스마트 제안"이다
100% 정확한 인식을 기대하면 실망한다. "제안 + 사용자 확인" 패턴으로 설계하면, 80% 정확도로도 사용자 경험이 크게 개선된다. 수동 입력 10개 → 수정 2개로 줄어드는 것만으로 충분하다.

### 도메인 지식이 프롬프트의 정확도를 결정한다
"사진에서 색상을 분석해줘"와 "볼더링 벽에서 홀드 색상을 분석해줘. 볼트와 테이프는 제외"는 결과가 완전히 다르다. 도메인을 아는 엔지니어가 프롬프트를 쓸 때 품질이 올라간다.

### MVP에서 커스텀 모델은 함정이다
"더 정확하게 하려면 모델을 직접 학습해야지"라는 생각은 사이드 프로젝트를 영원히 미완성으로 만든다. Vision API로 시작하고, 사용자가 충분히 모이면 그때 수정 데이터로 커스텀 모델을 학습해도 늦지 않다.
