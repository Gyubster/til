...
신용평가에 사용되는 CPS(Credit Profile System) 데이터는 외부 신용정보 기관의 API를 통해 조회한다. 772개의 코드를 요청하면 각 코드에 대한 응답값이 돌아오고, 이 데이터가 시스템 내 **7개의 서로 다른 소비자**에게 전달된다.
...
flowchart TD
    ...
    E --> G["ML 모델 A<br/>(SageMaker, 전체 772개)"]
...
```python
class Segment103(models.Model):
    class Meta:
        managed = False  # Django가 이 테이블의 스키마를 관리하지 않음
        db_table = 'tb_cr_kcb_0410432_103'
```
...
|--------|-------------|----------|--------|
| ML 모델 A (SageMaker) | 전체 772개 | 통째로 전달 | Dict |
