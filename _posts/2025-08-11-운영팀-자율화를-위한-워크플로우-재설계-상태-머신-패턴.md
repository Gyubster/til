...
flowchart TD
    subgraph as_is["AS-IS 프로세스"]
        A["채권관리팀: Retool에서 등록"] --> B["채권관리팀: Django Admin에서 상태 확인"]
        B --> C["채권관리팀 → 개발팀에 상환 처리 요청"]
        C --> D["개발팀: 스크립트 수동 실행"]
        D --> E["채권관리팀 → 개발팀에 종결 요청"]
        E --> F["개발팀: 금액 검증 후 종결 스크립트 실행"]
    end
    style as_is fill:#fff3cd,stroke:#856404
```

- 채권관리팀이 Retool과 Django Admin을 오가며 관리
...
flowchart TD
    subgraph to_be["TO-BE 프로세스"]
        A["채권관리팀: Retool에서 등록"] --> B["채권관리팀: Retool에서 상환 추가"]
        B --> C["시스템: 자동 익일 지급 처리"]
        C --> D["채권관리팀: 최종 상환 추가"]
        D --> E["시스템: 금액 검증 → 자동 종결"]
    end
    style to_be fill:#d4edda,stroke:#155724
```
